...
class SearchWordsPart extends SearchCurrentWord {
  caseSensitivityKind = 'SearchWordsPart'

  moveCursor (cursor) {
    if (this.input == null) {
      const wordRange = this.editor.getSelectedBufferRange()
      if (wordRange) {
  ...
  getPattern (term) {
    const escaped = this._.escapeRegExp(term)
    return new RegExp(escaped, this.isCaseSensitive(term) ? 'g' : 'gi')
  }
}
